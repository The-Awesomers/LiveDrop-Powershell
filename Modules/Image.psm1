...
Do { cls ; Get-Volume | Tee-Object -Append $temp | Out-Host   # Obtain list of connected Drives/Volumes
    $restart = "" <# Start to setup variables for final command. #>
    ...
    Write-Output "Are the file being saved to a shared/network folder or local volume? [ 0 = shared folder , 1 = local volume ] " | Tee-Object -Append $temp 
     ...
     If ($verify -ieq "y") {$path = test-path $destdir\WindowsImageBackup\$env:computername\Backup* -PathType Container} <# Check to see if a previous backup already exists in the destination directory. This process will replace it if it does.#>
        If ($path -eq $true) {Write-Host "A backup already exists for this PC at $destdir. If you want to save to this location, specify a subdirectory. This backup will be overwritten by this process. Are you absolutely certain you want to continue? (y\N)" | Tee-Object -Append $temp
            $chk1 = answer}
...
<#If (!"$destdir\WindowsImageBackup\$env:COMPUTERNAME") {#>mkdir -p "$destdir\WindowsImageBackup\$env:COMPUTERNAME\" #-ErrorAction SilentlyContinue} <# Creates directory for wbadmin to put the backup in. #>
$log = "$destdir\WindowsImageBackup\Image_Log_$logtime.txt"
Write-Output $temp
type $temp
Write-Output $log
Move-Item -Path "$temp" -Destination "$log" #-ErrorAction SilentlyContinue   # Move the file to final location
...
If ( $desttype -eq 0) {
    If ($validpas -eq "*" ) { $BSTR = [System.Runtime.InteropServices.Marshal]::SecureStringToBSTR($validpas)
     $unspass = "-password:"+ [System.Runtime.InteropServices.Marshal]::PtrToStringAuto($BSTR) }
<# ^^ Found this particular part online. It converts the password from a secure string to plaintext, temporarily. Could be exploited; use with caution. ^^ #>;
     ...
     wbadmin start backup -backupTarget:"$destdir\WindowsImageBackup\$env:COMPUTERNAME\" -include:$sourdir -user:$validusr $unspass $osfiles  | Tee-Object -Append $log 
...
ElseIf ( $desttype -eq 1 ) { wbAdmin start backup -backupTarget:$destdir -include:$sourdir $osfiles | Tee-Object -Append $log 
    Write-Output "A log file can be located at the following location $log." ; Done }
Else {Write-Output "Process cannot be executed. Please review previous parameters. A log file can be located at the following location $log." | Tee-Object -Append $log }
